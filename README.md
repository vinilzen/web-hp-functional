...
1. Please make sure you are using Node version 22.x.x LTS.
   1. You can check your node version by running `node -v` in your terminal.
   2. If you are using a different version, you can install the LTS version by following the instructions (Please use the LTS version) [here](https://nodejs.org/en/download).
2. Clone the repository and run `npm install` to install the dependencies.
3. Install playwright using `npx playwright install chromium --with-deps`.
4. Run `npm test` to run unit tests via vite.
5. Run `npm run playwright` to run integration test.
6. Run `npm run dev` to start the dev server.
   1. Please use a Chromium based browser to view the dev server.
